...
# count_light_roast <- function() {
#   coffee_clean_factors |>
#     select(age,roast_level) |>
#     drop_na() |>
#     filter(roast_level == "Light" | roast_level == "Medium" | roast_level == "Nordic") |>
#     count()
# }

young_roast_reg2 <- young_roast_reg |>
  group_by(age) |>
  summarize(prop_light = mean(roast_level_num))
...
Null hypothesis: The true proportion of preferring a lighter roast is the same between people in America aged 18-24 and people in America aged 25-34.

$$H_0: p_{\text{18-24}} - p_{\text{25-34}} = 0$$
...
$$H_a: p_{\text{18-24}} - p_{\text{25-34}} > 0$$
```{r}
...
```{r}
#source: https://infer.tidymodels.org/articles/anova.html
...
observed_f_statistic <- young_roast_reg %>%
  specify(roast_level_num ~ age) %>%
  hypothesize(null = "independence") %>%
...
```
The logistical regression model calculated the coefficients representing the that a person with less than a Bachelor's degree would on average have 0.06 less probability to choose less sugar additive types than a person with a Bachelor's degree or higher would.
...
$$H_0: p_{\text{bach}} - p_{\text{no_bach}} = 0$$
...
$$H_a: p_{\text{bach}} - p_{\text{no_bach}} != 0$$
```{r}
