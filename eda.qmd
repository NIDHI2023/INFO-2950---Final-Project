...
coffee_clean <- coffee_clean |>
  rename(
    ...
    prefer_between_abc = 
      "between_coffee_a,_coffee_b,_and_coffee_c_which_did_you_prefer?", 
...
colnames(coffee_clean) <- sapply(colnames(coffee_clean), function(name) {
  if (grepl("where_do_you_typically_drink_coffee", name)) {
    name <- gsub("where_do_you_typically_drink_coffee\\?_\\((.*)\\)", "drink_\\1",
                 name)
  } else if (grepl("how_do_you_brew_coffee_at_home", name)) {
    name <- gsub("how_do_you_brew_coffee_at_home\\?_\\((.*)\\)", "at_home_\\1",
                 name)
    ...
    name <- gsub("do_you_usually_add_anything_to_your_coffee\\?_\\((.*)\\)", 
                 "add_to_\\1", name)
  } else if (grepl("what_kind_of_dairy_do_you_add", name)) {
    name <- gsub("what_kind_of_dairy_do_you_add\\?_\\((.*)\\)", "dairy_add_\\1",
                 name)
...
coffee_clean_2 <- coffee_clean |>
  rename(at_home_coffee_brewing_machine = 
           `at_home_coffee_brewing_machine_(e.g._mr._coffee)`,
         ...
         purchase_national_chain = 
           `purchase_national_chain_(e.g._starbucks,_dunkin)`,
         ...
         sugar_sweetener_add_raw_sugar= 
           `sugar_sweetener_add_raw_sugar_(turbinado)`,
...
The observations represent an individual respondent to the survey. The columns are questions that they answered, ranging from demographic data to coffee preferences.
...
-Could assess relationship between caffeine, roast_level, best_described_before, and own_expertise to see how experience affects preferences, and if perceived preferences matches with actual preferences
-The ratio of satisfied to unsatisfied of getting good value for money is about the same more males and females, but nonbinary people have a higher ratio (also very likely because less sample)
