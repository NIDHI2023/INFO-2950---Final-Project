...
attributes such as age, gender, education level, and political identity?
...
#where respondents purchase their coffee
coffee_purchase_locations <- coffee_clean_factors |>
  gather(key = "location", value = "count", purchase_national_chain:purchase_other) |>
  group_by(location) |>
  summarise(total_count = sum(count, na.rm = TRUE))

ggplot(coffee_purchase_locations, aes(x = location, y = total_count, fill = location)) +
  geom_col() +
  labs(title = "Coffee Purchase Locations", 
       x = "Location", 
       y = "Count") +
  theme_minimal() +
  scale_fill_viridis_d(end = 0.8)
